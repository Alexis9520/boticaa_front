...
export async function getProductos() {
  return fetchWithAuth("http://localhost:8080/productos");
}

// Obtener historial de cajas
export async function getHistorial() {
  return fetchWithAuth("http://localhost:8080/api/cajas/historial");
}

// Agregar movimiento a caja
export async function agregarMovimiento(data: any) {
  return fetchWithAuth("http://localhost:8080/api/cajas/movimiento", {
    method: "POST",
...
export async function fetchWithToken() {
  ...
  return fetchWithAuth(`http://localhost:8080/api/cajas/actual?dniUsuario=${usuario.dni}`);
}
...
export async function actualizarProducto(codigoBarras: string, data: any) {
  return fetchWithAuth(`http://localhost:8080/productos/${codigoBarras}`, {
    method: "PUT",
    body: JSON.stringify(data),
    headers: { "Content-Type": "application/json" },
  });
...
export async function getStock() {
  return fetchWithAuth("http://localhost:8080/api/stock");
}

// Actualizar stock protegido por token
export async function actualizarStock(id: number, data: any) {
  return fetchWithAuth(`http://localhost:8080/api/stock/${id}`, {
    method: "PUT",
    body: JSON.stringify(data),
    headers: { "Content-Type": "application/json" },
  });
...
export async function getDashboardResumen() {
  return fetchWithAuth("http://localhost:8080/api/dashboard/resumen");
}